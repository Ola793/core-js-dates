...
function getCountDaysInMonth(month, year) {
  const currentMonthDate = new Date(Date.UTC(year, month - 1 + 1, 1));
  const previouseMonthDate = new Date(Date.UTC(year, month - 1, 1));
  const msInDay = 60 * 60 * 24 * 1000;

  return Math.round((currentMonthDate - previouseMonthDate) / msInDay);
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  return (Date.parse(dateEnd) - Date.parse(dateStart) + msInDay) / msInDay;
}
...
function getCountWeekendsInMonth(month, year) {
  const ammountOfDays = getCountDaysInMonth(month, year);
  let weekendCount = 0;

  for (let i = 1; i <= ammountOfDays; i += 1) {
    const date = new Date(Date.UTC(year, month - 1, i));
    const day = date.getUTCDay();

    if (day === 6 || day === 0) weekendCount += 1;
  }
...
function getWeekNumberByDate(date) {
  ...
  const dayNumber = originalDate.getUTCDay() || 7;
  originalDate.setUTCDate(originalDate.getUTCDate() + 4 - dayNumber);
  const yearStart = new Date(Date.UTC(originalDate.getUTCFullYear(), 0, 1));
  const msInDay = 60 * 60 * 24 * 1000;

  return Math.ceil(((originalDate - yearStart) / msInDay + 1) / 7);
}
